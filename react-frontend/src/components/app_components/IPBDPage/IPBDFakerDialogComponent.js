...
import iPBDFakerFactory from "./iPBDFakerFactory";
...
const IPBDFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = iPBDFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("iPBD").create(elem));
...
export default connect(mapState, mapDispatch)(IPBDFakerDialogComponent);
